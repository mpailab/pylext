...
bool shift(GrammarState &g, const LR0State &s, LR0State &res, int t, bool term) {
	...
	bool r = false;
	for (int i = 0; i < sz; i++) { // ƒл€ каждого сло€ в текущем состо€нии ищем, по какому ребру можно пройти по символу t
		auto &ed = term ? s.v[i].v->termEdges : s.v[i].v->ntEdges;
		auto it = ed.find(t); // TODO: заменить unordered_map на структуру с битовыми масками, по крайней мере, дл€ нетерминалов
		if ((it != ed.end())&&it->second->phi.intersects(s.v[i].M)) { // ѕровер€ем, проходит ли через следующую вершину хот€бы одно правило дл€ текущего множества нетерминалов s.v[i].M
			s.v[i].sh = it->second.get(); // ѕомечаем, что в i-м слое можно идти в следующую вершину
			r = true; // ѕомечаем, что сдвиг по нетерминалу t возможен
			k++;
		} else s.v[i].sh = 0;
	}
	if (!r)return false;
	...
	res.v[k].M.clear();
	for (int i = 0; i < sz; i++) {
		if ((*sv)[i].sh) { // ≈сли переход возможен в i-м слое
			auto *x = res.v[j].v = (*sv)[i].sh; // ƒобавл€ем в новый фрейм соответствующий слой
			res.v[j].M = (*sv)[i].M;
			for (auto &nn : x->ntEdges)
				if (nn.second->phi.intersects((*sv)[i].M))
					res.v[k].M |= g.tf.fst[nn.first]; // ƒобавл€ем в новый слой те нетерминалы, которые могут начинатьс€ в данной позиции в i-м слое
			j++;
		}
	}
	if (res.v[k].M.empty())
...
bool reduce(GrammarState &g, SStack &ss, PStack& sp, int a) {
	...
	for (int i = 1; i < F.size(); i++) {
		...
		if (A0 >= 0) {
			...
			for (int j = i; j > 0; j = k) {
				const NTTreeNode *u = 0;
				for (auto &p : B[j]) { // ѕеребор по обратным стрелкам из j-го фрейма вверх. —мотрим, по кака€ из стрелок соответствует свЄртке нетерминала A0
					if (p.M.has(A0)) {
