...
class VectorCmp {
	...
	V2 v2;
public:
	using value_type = V1::value_type;

	template<class T, bool str>
	struct RefBase {};
	template<class T>
	struct RefBase<T, true> : public T {};
	using RBase = RefBase<value_type, std::is_class_v<value_type>>;

	struct reference : public RBase {
		VectorCmp &v;
		int pos;
		reference(VectorCmp &x, int i) :v(x), pos(i) {}
		operator const value_type&() const { 
			Assert(v.v1[pos] == v.v2[pos]);
			return v.v1[pos];
		}
	...
	public:
		...
		template<class Z, class = std::enable_if<std::is_convertible<Z*, Y*>::value>::type>
		_iterator(const _iterator<Z>&i) :_ptr(i._ptr) {}
		_iterator() :_ptr(0) {}
		pointer operator->() { return _ptr; }
		reference operator*() { return *_ptr; }
