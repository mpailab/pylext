...
struct _Print_Num {
    ...
    template<class X>
    static void print_formatted_float(std::string& buf, X x, int width, int precision, char fmt, char sign, char sep, bool alt) {
        //throw FormattedPrintError("Floating point formatting not implemented");
        buf += std::to_string(x);
    }
};
template<>
struct _Print_Num<false> {
    template<class I>
    static void print_formatted_integer(std::string &buf, I i, int width, char fmt, char sign, char sep, bool alt) {
...
void print_formatted_int(std::string& buf, I i, int width, char fmt, char sign, char sep, bool alt) {
    if(sizeof(I)==1&&!fmt)
        fmt='c';
    _Print_Num<std::is_integral<I>::value>::print_formatted_integer(buf, i, width, fmt, sign, sep, alt);
}
...
void print_formatted_float(std::string& buf, X x, int width, int precision, char fmt, char sign, char sep, bool alt) {
    ...
    _Print_Num<is_fp>::print_formatted_float(buf, x, width, precision, fmt, sign, sep, alt);
}
...
void print_formatted(std::string& buf, const T& x, char fmt) {
    ...
    ss << x;
    buf = ss.str();
    //throw FormattedPrintError(std::string("formatted print not implemented for type ")+typeid(T).name());
}
        ...
        for (; pos < format.size(); pos++){
            if (format[pos] == '{') {
                if (format[pos + 1] != '{') {
                    ...
                    _parse_fmt(fmt, fmt_len, &align, &width, is_fp ? &precision : 0,
                               is_num ? &sign : 0, is_num ? &alt : 0, &fill, is_num ? &sep : 0,
                               &fmt0, &custom);
                    buf.clear();
                    if(custom){
