...
template<class C>
concept Container = requires(C x) {
	{ x.begin() };
	{ x.end() };
	{ x.size() };
};
...
template<class I>
concept Integer = std::is_integral_v<I>;

template<class X, class Y>
concept ConvertibleTo = std::is_convertible_v<X, Y>;

template<class N>
concept TreeNodeType = requires(N n, int i){
	{ n.used } -> Integer;
	{ n.refs } -> Integer;
	{ n.ch } -> Container;
	{ n[i] } -> ConvertibleTo<N>;
